...
int main(int argc, char *argv[]) {
	...
	if (argc > 1) {
		...
		fprintf(stderr, "You have not specify anything. Exiting...\n");
		return 1;
	}

	if (!key_given && pass == "") {
		fprintf(stderr, "You give not enought information. Try -h. Exiting...\n");
		return 1;
	}

	if (new_keyfilename != "") {
		if (pass == "") {
			fprintf(stderr, "You have not give passphrase. Exiting...\n");
			return 1;
		}
		else {
			FILE* new_keyfile; 
        		if ((new_keyfile = fopen(new_keyfilename.c_str(), "wb")) == NULL) {
				fprintf(stderr, "Error openeing new keyfile. Exiting..n");
				return 1;
			}
			unsigned long long key[4];
			CREATE_KEY(pass, key);
	...
	if (pass != "") {
		...
		FILE* keyfile;
			...
			fprintf(stderr, "Error openeing keyfile. Exiting...\n");
			return 1;
		}
	...
	if (!flag) {
		...
		if ((random = fopen("/dev/urandom", "rb")) == NULL) {
			fprintf(stderr, "Error opening urandom. Exiting...\n");
			return 1;
		}
